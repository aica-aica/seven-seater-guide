...
export default function SeatingDiagram({ seating, modelName }: SeatingDiagramProps) {
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 sm:p-6 shadow-xl text-slate-200">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between pb-4 border-b border-slate-800 gap-3">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            佈局規格：<strong className="text-amber-400">{seating.layout}</strong> ({seating.layoutDescription})
          </p>
        </div>
      ...
      {/* Rail Type Banner */}
      <div className="my-4 p-3 rounded-xl bg-slate-950/90 border border-slate-800 text-xs text-slate-300 flex items-center gap-2">
        <span className="text-amber-400 font-bold">滑軌機構：</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-2 items-center">
        ...
        <div className="bg-slate-950 p-6 rounded-2xl border border-slate-800/80 flex flex-col items-center">
          ...
          {is223 ? (
            <div className="flex justify-between w-48 gap-4 mb-4 relative">
              <div className="w-20 h-16 bg-amber-500/20 rounded-xl border border-amber-500/50 flex flex-col items-center justify-center text-[10px] text-amber-300 font-bold shadow-md">
                獨立座
                <span className="text-[8px] text-amber-400/80 font-normal">ISOFIX</span>
              </div>
              {/* Walk-through Aisle */}
              <div className="flex flex-col items-center justify-center text-[9px] text-amber-400 font-mono">
                <span>走道</span>
            ...
            <div className="flex justify-center w-52 gap-1.5 mb-4">
              <div className="w-16 h-16 bg-slate-800 rounded-xl border border-slate-700 flex flex-col items-center justify-center text-[10px] text-slate-300 font-semibold">
                二排左
                <span className="text-[8px] text-amber-400">ISOFIX</span>
              </div>
        ...
        <div className="space-y-4 text-xs">
          <div className="p-4 rounded-xl bg-slate-950/60 border border-slate-800">
            ...
            <p className="text-slate-300 leading-relaxed">
              {seating.secondRowWalkThroughWidthMm > 0 ? (
                <>
                  走道淨寬達 <strong className="text-amber-400 font-mono text-sm">{seating.secondRowWalkThroughWidthMm} mm</strong>。即使第二排固定安裝兩張大型兒童安全座椅，成人仍可直接穿過中央走道進入第三排，完全不需前翻座椅。
                </>
              ) : (
                <>
          ...
          <div className="p-3.5 rounded-xl bg-slate-950/60 border border-slate-800 flex items-start gap-2">
