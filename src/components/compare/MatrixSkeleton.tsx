...
export default function MatrixSkeleton() {
  return (
    <div className="space-y-6 animate-pulse" aria-label="規格矩陣表載入中...">
      ...
      {/* Table Skeleton */}
      <div className="overflow-x-auto rounded-2xl border border-white/[0.08] bg-[#0e1424]/80 shadow-2xl">
        <div className="min-w-[700px]">
          {/* Table Header Row Skeleton */}
          <div className="grid grid-cols-4 border-b border-slate-800 bg-slate-900/80 p-4 gap-4">
            <div className="h-8 w-24 bg-slate-800 rounded-md" />
            {[1, 2, 3].map((i) => (
              <div key={i} className="space-y-2">
                <div className="h-28 bg-slate-800/70 rounded-xl" />
          ...
          <div className="divide-y divide-slate-800/60 p-4 space-y-4">
            {[1, 2, 3, 4, 5, 6, 7].map((row) => (
              <div key={row} className="grid grid-cols-4 gap-4 pt-4 items-center">
                ...
                <div className="h-6 w-3/4 mx-auto bg-slate-800/60 rounded-md" />
                <div className="h-6 w-3/4 mx-auto bg-slate-800/60 rounded-md" />
                <div className="h-6 w-3/4 mx-auto bg-slate-800/60 rounded-md" />
              </div>
            ))}
