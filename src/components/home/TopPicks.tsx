    ...
    <section className="py-16 border-b border-slate-200/80 text-slate-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-12">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-sky-50 text-sky-700 text-xs font-semibold border border-sky-200/80 mb-3 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {topPicks.map((pick) => {
            ...
            return (
              <div
                key={pick.carId}
                className="bg-white/90 backdrop-blur-xl border border-slate-200/90 rounded-2xl p-5 flex flex-col justify-between hover:border-cyan-500/40 transition-all hover:-translate-y-1 shadow-[0_4px_20px_rgba(15,23,42,0.05)] hover:shadow-[0_10px_30px_rgba(6,182,212,0.1)] group"
              >
                <div>
                  ...
                  <div className="h-36 rounded-xl overflow-hidden mb-4 bg-slate-100 border border-slate-200/80">
                    <img
                  ...
                  <p className="text-xs text-cyan-700 font-bold font-mono mb-3">
                ...
                <div className="pt-4 border-t border-slate-200/80">
                  <span className="text-[11px] text-slate-500 block mb-3 leading-tight">
                    <strong className="text-slate-700">適合對象：</strong> {pick.targetAudience}
                  </span>
                  <Link
                    href={`/cars/${car.slug}`}
                    className="w-full py-2 px-3 rounded-xl bg-slate-50 hover:bg-gradient-to-r hover:from-cyan-600 hover:to-blue-600 hover:text-white text-cyan-700 border border-slate-200/90 text-xs font-bold flex items-center justify-center gap-1.5 transition-all shadow-sm"
                  >
                    查看完整實測
